...
<div align="center" padding=25px>
    <img src="images/cdc-connector-5.png" width=50% height=50%>
</div>
...
```sql
CREATE TABLE TABLE_CUSTOMER  WITH(KAFKA_TOPIC ='TABLE_CUSTOMER',VALUE_FORMAT = 'AVRO')
AS SELECT
...
FROM TRANSACTIONS t
INNER JOIN TABLE_CUSTOMER cust ON t.CREDIT_CARD_NUMBER = cust.CREDIT_CARD_NUM
EMIT CHANGES;                                   
```

```sql
CREATE TABLE FEATURE_SET
WITH (KAFKA_TOPIC='FEATURE_SET',VALUE_FORMAT='JSON_SR')
AS SELECT 
    CREDIT_CARD_NUMBER,
    LATEST_BY_OFFSET(CUSTOMER_EMAIL) AS CUSTOMER_EMAIL,
